<?php
$num_of_per = count($pertanyaanlatihan);
$count = 1;
?>
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div>
            ...
            <form id="anserForm" action="<?= site_url('murid'); ?>" method="post">
              <div class="question_content">
                <input type="hidden" name="latihan_id" value="<?= $latihan->title_id; ?>">
                <input type="hidden" name="token" value="<?= time(); ?>">
                <div id="Carousel" class="carousel" data-interval="false" data-wrap="false" >
                  <div class="carousel-inner">
                    <?php
                    foreach ($pertanyaanlatihan as $per): $type = ($per->option_type == 'Radio') ? 'radio' : 'checkbox'; ?>
                    <div class="item <?= ($count == $num_of_per) ? 'active' : '' ?>">
                      <div class="col-xs-9">
                        <h3><?= $per->pertanyaanlatihan ?></h3><br/>
                        ...
                        ?>
                        <?php
                        foreach ($jawabanlatihan[$per->per_id][0] as $jaw) { ?>
...
$(document).ready(function() {
  ...
  function timer() {
    ...
    time += count * 1000;
    now.setTime(time);
    document.cookie="latihanTimeDuration="+count+"; expires="+now.toUTCString()+"; path=/";
