...
class Saran extends CI_Controller
{
	...
	public function save()
	{
		...
		}else
		{
			...
			{
				$config['upload_path']='./assets/upload/saran/';
				...
				}else
				{
					...
					$title_id=$this->Saran_m->add_kursus_title($upload_data['file_name']);
				}
			}else
			{
				$title_id=$this->Saran_m->add_kursus_title();
			}
			if ($title_id) 
			{
	...
	public function update_kursus($id,$pesan='')
	{
		...
		if ($this->form_validation->run()==FALSE) 
		{
			$this->Saran_m->update_kursus_title($id);
		}else
		{
			$form_info=array();
			if (!empty($_FILES['feature_image']['name']) )
			{
				...
				{	
					$upload_data=$this->upload->data();
					$title_id=$this->Saran_m->update_kursus_title($id,$upload_data['file_name']);
				}
			}else
			{
				$title_id=$this->Saran_m->update_kursus_title($id);
			}
			if($title_id){
				redirect('saran','refresh');
			}else{
	...
	{
		 $sub_cat = $this->Saran_m->get_subcategori_by_cat_id($id);
        $str = '';
	...
	public function kursusdelete($id)
	{
		...
		if (!empty($have_sections)) 
		{
			...
			$this->session->set_flashdata('message',$message);
			redirect('Saran/'.$kursus_id,'refresh');
		}
	}
	public function kursusdetail($id='', $pesan='')
	{
		...
		$this->data['kursus']=$this->Saran_m->get_kursus_detail($id);
		$this->data['sections']=$this->Saran_m->get_sections($id);
		$this->data['halaman']="vkursusdetail";
		$this->load->view('_main',$this->data);
	}
	...
	{
		{
			...
			$this->data['kursus_title']=$this->db->get_where('saran',array("kursus_id"=>$kursus_id))->row()->kursus_title;
			$this->data['halaman']='addsection';
			$this->load->view('_main',$this->data);
		}
	}
	public function save_sections()
	{
		...
		{
			if ($this->Saran_m->save_sections_m()) 
			{
				//$message="sukses";
				//$this->session->set_flashdata('message',$message);
				$this->data['sections']=$this->Saran_m->get_sections($this->input->post('kursus_id'));
				redirect(site_url('saran/kursusdetail/'.$this->input->post('kursus_id')),$this->data);
	...
	{
		$this->data['sections']=$this->Saran_m->get_sections($kursus_id);
		$this->data['kursus_id']=$kursus_id;
		$this->data['pesan']=$pesan;
		$this->data['kursus_title']=$this->db->get_where('saran',array('kursus_id' =>$kursus_id))->row()->kursus_title;
		$this->data['halaman']='vaddvideo';
		$this->load->view('_main',$this->data);
	}
	public function upload_kursus_videos($pesan='')
	{
		...
		}else
		{
			...
			if(!empty($_FILES['media']['name']))
			{
				...
				if(!is_dir('assets/upload/saran/video/'.$directory))
				{
					mkdir('./assets/upload/saran/video/'.$directory,0777,TRUE);
				}
				$config['upload_path']='./assets/upload/saran/video/'.$directory.'/';
				...
				if(!$this->upload->do_upload('media'))
				{
					...
					redirect(site_url('saran/addvideo/'.$this->input->post('kursus_id')));
				}else
				{
					$upload_data=$this->upload->data();
					$video_id=$this->Saran_m->add_video_m($upload_data['file_name']);
				}
			}
			if($video_id)
			{
	...
	public function upload_kursus_document($pesan='')
	{
		...
		}else
		{
			...
			if(!empty($_FILES['media']['name']))
			{
				...
				$extensi=$path_parts['extension'];
				$directory=$this->input->post('kursus_id');
				if(!is_dir('assets/upload/saran/document/'.$directory))
				{
					mkdir('./assets/upload/saran/document/'.$directory,0777,TRUE);
				}
				$config['upload_path']='./assets/upload/saran/document/'.$directory.'/';
				...
				if(!$this->upload->do_upload('media'))
				{
					$error=$this->upload->display_errors();
					$this->session->set_flashdata('pesan', $error);
					redirect(site_url('saran/adddocument/'.$this->input->post('kursus_id')));
				}else
				{
					$upload_data=$this->upload->data();
					$document_id=$this->Saran_m->add_document_m($upload_data['file_name']);
				}
			}
			if($document_id)
			{
	...
	{
		if(!is_numeric($id)){show_404();}
		$isimateri=$this->db->get_where('saran_sections', array('kursus_id'=>$id))->result();
		if(!empty($isimateri))
		{
			$pesan='modul ini mempunyai materi yang telah diupload, silahkan hapus dulu materi ';
			$this->session->set_flashdata('pesan', $pesan);
			redirect(site_url('saran/kursusdetail/'.$isimateri[0]->kursus_id));
		}else
		{
			$kursus_id=$this->db->get_where('saran',array('kursus_id'=>$id))->row()->kursus_id;
			$this->db->where('kursus_id',$id);
	...
	public function hapusisi_modul($id)
	{
		...
		$adadoc=$this->db->get_where('saran_document',array('section_id'=>$id))->result();
		if (!empty($adadoc)) 
		{
			$pesan='isi modul ini mempunyai Document, silahkan hapus document nya dulu';
			$this->session->set_flashdata('pesan',$pesan);
			redirect(site_url('saran/kursusdetail/'.$adadoc[0]->kursus_id));
		}else
		{
			$kursus_id=$this->db->get_where('saran_sections',array('section_id'=>$id))->row()->kursus_id;
			$this->db->where('section_id',$id);
			$this->db->delete('saran_sections');
			if($this->db->affected_rows()==1)
			{
				$pesan='sukses';
			}else
			...
			redirect(site_url('saran/kursusdetail/'.$kursus_id));
		}
	}
	public function section_detail($id,$pesan='')
	{
		...
		$data=array();
		$this->data['pesan']=$pesan;
		$this->data['materi']=$this->Saran_m->get_materi_detail_m($id);
		
		$this->data['doc']=$this->Saran_m->get_materi_doc_m($id,$this->data['materi']->kursus_id);
		$this->data['halaman']='vmateridetail';
		$this->load->view('_main',$this->data);
	}
	public function delete_document($id)
	{
		...
		$document=$this->db->where('document_id',$id)->get('saran_document')->row();
		if($level !='superadmin')
		{
			$author=$this->db->where('kursus_id',$document->kursus_id)->get('saran')->row()->created_by;
			if($author != $user_id)
			{
				exit('kamu tidak memiliki akses kesini');
			}
		}
		$this->db->where('document_id',$id)->delete('saran_document');
		if(unlink('saran_document/'.$document->kursus_id.'/'.$document->document_link))
		{
		...
		$this->session->flashdata('pesan',$pesan);
		redirect(site_url('saran/section_detail/'.$document->section_id));
	}
	public function adddocumentmateri($section_id='',$pesan='')
	{
		...
		$this->data['kursus']=$this->Saran_m->get_kursus($section_id);
		$this->data['kursus_id']=$this->db->get_where('saran_sections',array('section_id'=>$section_id))->row()->kursus_id;
		$kursus_id=$this->data['kursus_id'];
		$this->data['kursus_title']=$this->db->get_where('saran',array('kursus_id'=>$kursus_id))->row()->kursus_title;
		$this->data['section_name']=$this->db->get_where('saran_sections',array('section_id'=>$section_id))->row()->section_name;
		$this->data['halaman']='vadddocumentmateri';
		$this->load->view('_main',$this->data);
	...
	public function upload_kursus_document_materi($pesan='')
	{
		...
		}else
		{
			...
			if($document_id)
			{
				$sukses='materi berhasil di upload';
					$this->session->set_flashdata('pesan', $sukses);
					redirect(site_url('saran/section_detail/'.$this->input->post('section')));
			}else
			{
				echo "as";
			}
		...
		if($this->form_validation->run()==FALSE){
			echo "gagagl";
				...
				$this->load->library('upload');
       			$nmfile = "file_".time();
        		$path   = './assets/upload/saran/';
        		$config['upload_path'] = $path;
        		$config['allowed_types'] = 'docx|doc|pdf|txt|xls';
        		$config['overwrite']=TRUE;
     			$config['max_size'] = '3000000000000000000000000000';
	...
	public function update_materi()
	{
			...
			if($this->form_validation->run()==FALSE)
			{
				redirect('saran/edit_kursus_detail/$id','refresh');
			}else
			{
	            $config = array(
                'file_name' => "file_".time(),
                'upload_path' => "./assets/upload/saran/",
            	...
            	if ($this->upload->do_upload('document')) {
            	$user_data = $this->db->get_where('saran', array('kursus_id' => $kursus_id))->row();
            	$path = './assets/upload/saran/'.$user_data->document;
      			  if (!empty($path)) {
            	unlink($path);
      			}
                ...
                $data = array(
          	   ...
          	   $this->Saran_m->actionupdate($kursus_id, $data);
	...
	public function hapus_materi($id)
	{
        ...
        $user_data = $this->db->get_where('saran', array('kursus_id' => $id))->row();
        $path = './assets/upload/saran/'.$user_data->document;

        if (!empty($path)) {
            unlink($path);
        }
        $this->Saran_m->deletemateri($id);
