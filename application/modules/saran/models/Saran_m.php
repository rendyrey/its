...
class Saran_m extends CI_Model { 
    ...
    public function getall()
    {
        $result =$this->db->select('*')
                ->from('saran')
                ->join('categori','categori.categori_id = saran.categori_id','left')
               // ->join('categori','subcategori.cat_id = categori.categori_id','left')
                //->join('user','user.id=saran.created_by')
                ->get()
                ->result();
        return $result;
    }
    ...
    public function add_kursus_title($upload_data='')
    {
       ...
       // $info['kursus_price'] = $this->input->post('price', TRUE);
        ...
        $if_exist = $this->db->get_where('saran', array('kursus_title' => $info['kursus_title']), 1)->result();
        if ($if_exist) {
            return FALSE;
        } else {
            $this->db->insert('saran', $info);
       ...
       $sections = $this->input->post('section', TRUE);
        ...
        foreach ($sections as $value) {
            if ($value != '') {
                ...
                $this->db->insert('saran_sections', $info);
                $i++;
            }
        }
        return TRUE;
    ...
    public function add_video_m($upload_data)
    {
        ...
        $if_exist=$this->db->get_where('saran_videos',array('video_title' =>$info['video_title']),1)->result();
        if($if_exist)
            {
                return FALSE;
            }
            ...
            {
                $this->db->insert('saran_videos',$info);
                if($this->db->affected_rows()==1)
                {
                    $video_id=$this->db->insert_id();
                    $section_video_ids=$this->db->get_where('saran_sections',array('section_id'=>$info['section_id']))->row()->video_ids;
                    ...
                    $this->db->where('section_id',$info['section_id'])->update('saran_sections',$data);
                    return $video_id;
                }else
                {
                    return FALSE;
    ...
    public function add_document_m($upload_data)
    {
        ...
        $if_exist=$this->db->get_where('saran_document',array('document_title' =>$info['document_title']),1)->result();
        if($if_exist)
            {
                return FALSE;
            }
            ...
            {
                $this->db->insert('saran_document',$info);
                if($this->db->affected_rows()==1)
                {
                    $video_id=$this->db->insert_id();
                    $section_video_ids=$this->db->get_where('saran_sections',array('section_id'=>$info['section_id']))->row()->video_ids;
    ...
    public function get_materi_detail_m($id)
    {
        $hasil  = $this->db->select('*')
                ...
                ->join('saran','saran.kursus_id = saran_sections.kursus_id')
                ->get()
                ->row();
        return $hasil;
    }
    ...
    public function ambil_materi()
    {
        $result=$this->db->select('*')
                ->from('saran_document')
                ->join('saran_sections','saran_sections.section_id = saran_document.section_id ','left')
                ->join('saran','saran_sections.kursus_id = saran.kursus_id','left')
                ->join('categori','categori.categori_id = saran.categori_id','left')
